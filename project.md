...
* 2026-05-24：项目最低运行环境固定为 Python `>=3.12`；运行依赖下限调整为 Matplotlib `>=3.8.0`、NumPy `>=1.26.0`、Pillow `>=11.0.0`，用于覆盖明确支持 Python 3.12 的最低主线版本；`uv` 解析策略固定为 `lowest-direct`，确保常规同步环境也优先安装这些直接依赖下限。
